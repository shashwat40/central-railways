...
if(isset($_SESSION['username'])) {
      ...
      if(isset($_POST['subemp'])) {
            $emp_no = $_POST['empno'];
            $cga_list = mysqli_query($connection, "SELECT * FROM remarks WHERE emp_no = '$emp_no'");
      } else if(isset($_POST['subdate'])) {
            $date = $_POST['searchdate'];
            $cga_list = mysqli_query($connection, "SELECT * FROM remarks WHERE DATE_FORMAT(remark_date, '%Y-%m-%d') = DATE_FORMAT('$date', '%Y-%m-%d')");
      } else {
            $cga_list = mysqli_query($connection, "SELECT * FROM remarks");
      }
      while($row = mysqli_fetch_array($cga_list)) {
            ...
            echo "<div class='panel'>
                  <p>
                  <br> Remark By :          " . $row['remark_by'] . "
                  <br> Remark Date :        " . $row['remark_date'] . "
